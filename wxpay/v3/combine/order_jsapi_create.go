...
type JsapiQueryParams struct {
	...
	SceneInfo         *SceneInfoObject        `json:"scene_info"`           // 支付场景信息描述
	SubOrders         []*SubOrdersObject      `json:"sub_orders"`           // 子单信息，最多支持子单条数：50
	CombinePayerInfo  *CombinePayerInfoObject `json:"combine_payer_info"`   // 支付者信息
	TimeStart         string                  `json:"time_start"`           //订单生成时间，遵循rfc3339标准格式
	TimeExpire        string                  `json:"time_expire"`          //订单失效时间，遵循rfc3339标准格式
	NotifyUrl         string                  `json:"notify_url"`           //接收微信支付异步通知回调地址，通知url必须为直接可访问的URL，不能携带参数。 示例值：https://yourapp.com/notify
...
type SceneInfoObject struct {
	...
	PayerClientIp string `json:"payer_client_ip"` //必填。用户端实际ip
}
...
type SubOrdersObject struct {
	...
	//SubMchid      string            `json:"sub_mchid"` //普通商户不允许传sub_mch_id
	Detail        string            `json:"detail"`
	ProfitSharing bool              `json:"profit_sharing"`
	Description   string            `json:"description"`
	SettleInfo    *SettleInfoObject `json:"settle_info"`
}
